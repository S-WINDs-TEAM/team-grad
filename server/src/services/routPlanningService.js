...
const computeWaypointsForDeparture = async (
  ...
  for (const wp of rawWaypoints) {
    ...
    if (segmentDistanceKm > 0) {
      const lastmaxSafeSpeed =
        waypointsWithWeather.length > 0
    ...
    if (cached) {
      ...
      await delay(50);
      weather = await getWeatherForLocationAndTime(
        wp.lat,
        wp.lng,
        etaForThisPoint,
      );
    ...
    const fuelRaw = calculateFuelImpact(
      weather,
      vehicleType,
      "medium",
      speeds[vehicleType] || 90,
    );
    const fuelImpact =
      fuelRaw && typeof fuelRaw === "object"
        ? fuelRaw
  ...
  const totalDistance =
    rawWaypoints[rawWaypoints.length - 1]?.distanceFromStart || 0;
  const targetCount = Math.max(2, Math.ceil(totalDistance / 30)); // at least 2 points
  const step = Math.max(
    1,
    Math.floor(waypointsWithWeather.length / targetCount),
  );
  ...
  const avgRiskScore =
    waypointsWithWeather.reduce(
      (sum, wp) => sum + riskScores[wp.weather.riskLevel],
      0,
    ) / waypointsWithWeather.length;
  const overallRisk =
    avgRiskScore > 2.2 ? "high" : avgRiskScore > 1.4 ? "medium" : "low";
  const totalDurationMin =
    (cumulativeEta.getTime() - departure.getTime()) / (60 * 1000);
...
const rebuildWaypointsForPolyline = async (
  polyline,
  totalDistanceKm,
  vehicleType,
  departure,
  ...
  return computeWaypointsForDeparture(
    rawWaypoints,
    vehicleType || "car",
    departure || new Date(),
  );

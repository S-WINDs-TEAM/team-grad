...
const {
  getRouteAndWaypoints,
  computeWaypointsForDeparture,
} = require("../services/routPlanningService");
...
const {
  computeAlternateCandidates,
  applyAlternate,
} = require("../services/alternateService");
// ================================================================
...
const planRoute = async (req, res, next) => {
  try {
    ...
    const { rawWaypoints, routePolyline, route } = await getRouteAndWaypoints(
      origin,
      destination,
    );
    const {
    ...
    const fatigueInfo = await evaluateFatigue(
      driverIdForFatigue,
      totalDurationMin,
    );
    ...
    const riskScore = Math.round(
      fullWaypoints.reduce((s, w) => s + (w.riskScore || 0), 0) /
    ...
    const highRiskWaypoint = fullWaypoints.find(
      (wp) => wp.weather.riskLevel === "high",
    );
    let alertId = null;
    ...
    if (highRiskWaypoint) {
      ...
      const alternate = await calculateLocalAlternate(
        origin,
        destination,
        dangerWp,
        10,
      ...
      const timeSaved = Math.round(
        currentRouteSummary.durationMin - proposedRouteSummary.durationMin,
      );
      const distanceDiff =
        Math.round(
          (proposedRouteSummary.distanceKm - currentRouteSummary.distanceKm) *
            10,
        ) / 10;
      const waypointIndex = fullWaypoints.findIndex(
        (wp) =>
          wp.location.lat === highRiskWaypoint.location.lat &&
          wp.location.lng === highRiskWaypoint.location.lng,
      );
      // Context so the manager knows WHICH vehicle/driver/route instantly
      let ctxVehicle = null;
      if (vehicleIdForAlert) {
        ctxVehicle = await FleetVehicle.findById(vehicleIdForAlert).populate(
          "driverId",
          "name",
        );
      }
      ...
      if (req.user.companyId) {
        ...
        for (const m of managers) {
          await notify({
            ...
            metadata: {
              tripId: trip._id,
              alternateRoute: alternateRouteData,
              context,
              reason: "auto-detected hazard",
            },
          });
        }
      }
  ...
  try {
    ...
    for (const candidateDeparture of candidateTimes) {
      const { overallRisk, avgRiskScore, totalDurationMin, arrivalTime } =
        await computeWaypointsForDeparture(
          rawWaypoints,
          vtype,
          candidateDeparture,
        );
    ...
    suggestions.sort((a, b) => {
      if (a.avgRiskScore !== b.avgRiskScore)
...
const getTripById = async (req, res, next) => {
  try {
    ...
    const trip = await Trip.findOne(query).populate({
      path: "vehicleId",
...
const applyAlternateRoute = async (req, res, next) => {
  try {
    ...
    if (!candidate?.coordinates?.length)
      return res
        .status(400)
        .json({ success: false, msg: "candidate is required" });
    const trip = await applyAlternate(req.params.tripId, candidate);
    res
      .status(200)
      .json({ success: true, msg: "alternate applied", tripId: trip._id });
  } catch (err) {
    if (err.statusCode)
      return res

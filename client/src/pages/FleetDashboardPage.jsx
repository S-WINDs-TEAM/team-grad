...
import {
  startMockTrackingApi,
  stopMockTrackingApi,
} from "../api/mockTrackingApi";
...
const FleetDashboardPage = () => {
  ...
  const handleAssignDriver = async (vehicleId, driverId) => {
    try {
      ...
      setFleetWithTrips((prev) =>
        prev.map((item) =>
          item.vehicle.id === vehicleId
            ? {
                ...item,
                vehicle: { ...item.vehicle, driverId: updatedVehicle.driverId },
              }
            : item,
        ),
      );
              ...
              {alternatePreview && (
                <div style={local.altPreviewCard}>
                  ...
                  <div style={local.altPreviewMeta}>
                    {alternatePreview.distanceKm?.toFixed(1)} km ·{" "}
                    {Math.round(alternatePreview.durationMin)} min · saves{" "}
                    {alternatePreview.timeSaved} min
                  </div>
                  <div style={local.altPreviewActions}>
                    <button
                      style={local.applyBtn}
                      onClick={handleApplyAlternate}
                    >
                      Apply alternate
                    </button>
                    <button
                      style={local.dismissBtn}
                      onClick={() => setAlternatePreview(null)}
                    >
                      Dismiss
                    </button>
                  </div>
            ...
            {alerts.map((a, i) => (
              <div
                ...
                style={{
                  ...
                  borderColor: a.details?.lat
                    ? theme.accentOrange
                    : styles.alertHistoryItem.borderColor,
                }}
              ...
              >
                ...
                {a.details?.context && (
                  <div style={local.alertContext}>
                    {a.details.context.plateNumber
                      ? `${a.details.context.plateNumber} · `
                      : ""}
                    {a.details.context.driverName} ·{" "}
                    {a.details.context.routeLine}
                  </div>
                )}
        ...
        <div style={styles.modalOverlay} onClick={() => setInviteLink(null)}>
          ...
          >
            ...
            <div style={styles.modalBody}>
              ...
              >
                Email sending is not connected yet — share this link with{" "}
                {inviteLink.email} directly. The link expires in 3 days.
              </p>
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                />
                <button
                  style={{ ...styles.miniBtn, whiteSpace: "nowrap" }}

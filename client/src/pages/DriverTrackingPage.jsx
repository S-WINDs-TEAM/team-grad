...
const DriverTrackingPage = () => {
  ...
  const sampledWaypoints = useMemo(() => {
    ...
    if (out[out.length - 1] !== all[all.length - 1])
  ...
  const handleBreak = async () => {
    ...
    try {
      ...
      toast.success(
        "break request sent — auto-approves in 7 minutes if no response",
      );
    } catch (err) {
  ...
  const handleAltRequest = async () => {
    ...
    setBusy("alt");
    try {
      await createRouteRequestApi({
        tripId: trip._id,
        reason: "Hazard reported on the current route",
      });
      toast.success(
        "alternate route request sent — waiting for manager decision",
      );
    } catch (err) {
      ...
      <div style={styles.content}>
        ...
        {vehicle ? (
          <div style={styles.vehicleCard}>
            <div style={styles.vehicleCardHeader}>
              ...
              <span
                style={{
                  ...styles.statusBadge,
                  color: getFleetStatusColor(vehicle.status),
                }}
              >
          ...
          !loading && (
            <div style={styles.noVehicleCard}>
              ...
              >
                <Route size={16} />{" "}
                {busy === "trip" ? "Sending…" : "Request a Trip"}
              </button>
            </div>
          )
        )}
          ...
          <>
            ...
            {hasHazard && (
              <div style={styles.hazardNote}>
                A high-risk weather point was detected on your route. You can
                request an alternate path.
              </div>
            )}
            ...
            {trip?.cargoAlerts?.length > 0 && (
              <div style={styles.cargoBox}>
                <div style={styles.cargoTitle}>
                  Cargo advisories for this trip
                </div>
        ...
        {vehicle && (
          <div style={styles.actionsRow}>
            ...
            >
              <Coffee size={16} />{" "}
              {busy === "break" ? "Sending…" : "Request Break (15 min)"}
            </button>
            {trip && hasHazard && (
              <button
                style={styles.altBtn}
                onClick={handleAltRequest}
                disabled={busy === "alt"}
              >
                <Route size={16} />{" "}
                {busy === "alt" ? "Sending…" : "Request Alternate Route"}
              </button>
            )}
          </div>
        )}
        ...
        {lastPosition && (
          <p style={styles.coords}>
            Last sent: {lastPosition.lat.toFixed(5)},{" "}
          ...
          {showChat && (
            <div style={styles.chatBox}>
              <p style={styles.safetyNote}>
                For safety: reply only while stopped.
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {m.message}
                        {m.tripId && (
                          <span style={styles.tripTag}>trip-linked</span>
                        )}
                      </div>
                    </div>
                  );

  ...
  useEffect(() => {
    ...
    if (showRoutes) {
      mergedFleetData.forEach((item) => {
        ...
        if (!trip || !trip.routePolyline || trip.routePolyline.length === 0)
          return;

        const isSelected = item._id === selectedVehicleId;
        const riskColor = getRiskPathColor(trip.overallRiskLevel);
        const positions = trip.routePolyline.map((coord) => [
          coord[0],
          coord[1],
        ]);
        ...
        if (trip.origin && trip.destination) {
          ...
          const startMarker = L.marker([trip.origin.lat, trip.origin.lng], {
            icon: startIcon,
          })
            .addTo(map)
    ...
    if (!showRoutes && selectedVehicleId) {
      const selectedItem = mergedFleetData.find(
        (item) => item._id === selectedVehicleId,
      );
      const selTrip = selectedItem?.todayTrip;
      if (
        selTrip &&
        selTrip.routePolyline &&
        selTrip.routePolyline.length > 0
      ) {
        const riskColor = getRiskPathColor(selTrip.overallRiskLevel);
        const positions = selTrip.routePolyline.map((coord) => [
          coord[0],
          coord[1],
        ]);
        const polyline = L.polyline(positions, {
    ...
    if (selectionChanged) {
      if (selectedVehicleId) {
        ...
        if (
          selectedItem &&
          selectedItem.todayTrip &&
          selectedItem.todayTrip.routePolyline.length > 0
        ) {
          const positions = selectedItem.todayTrip.routePolyline.map(
  ...
  useEffect(() => {
    ...
    if (alternatePreview?.polyline?.length) {
      const line = L.polyline(alternatePreview.polyline, {
        ...
        .addTo(map)
        .bindPopup("Proposed alternate route");
    ...
    if (hazardFocus) {
      ...
      const m = L.marker([hazardFocus.lat, hazardFocus.lng], { icon })
        .addTo(map)
        .bindPopup("Weather hazard location");
      focusLayers.push(m);

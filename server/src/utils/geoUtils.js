...
const getVehicleProfile = (vehicleType, vehicleHeight = 'medium') => {
    const profiles = {
        car: {
            baseSpeed: 120,
            windSensitivity: 1.0,
            heightFactor: vehicleHeight === 'high' ? 0.6 : vehicleHeight === 'medium' ? 0.4 : 0.3,
        ...
        truck: {
            baseSpeed: 90,
            windSensitivity: 1.8,
            heightFactor: vehicleHeight === 'high' ? 0.8 : vehicleHeight === 'medium' ? 0.6 : 0.5,
            fuelEfficiency: 25.0,
        },
        bus: {
            baseSpeed: 80,
...
const calculateWindImpact = (windSpeed, vehicleProfile) => {
    ...
    const windReduction = (windSpeed * vehicleProfile.windSensitivity * vehicleProfile.heightFactor) / 2;
...
const calculateRiskLevel = (weatherData, vehicleType, vehicleHeight = 'medium', roadMaxSpeed = 120) => {
    const { condition, windSpeed, visibility, precipitation, windGust } = weatherData;

    // حساب السرعة الآمنة باستخدام الدالة المطورة
    const maxSafeSpeed = calculateMaxSafeSpeed(weatherData, vehicleType, vehicleHeight, roadMaxSpeed);
...
const calculateFuelImpact = (weatherData, vehicleType, vehicleHeight = 'medium', speed) => {
    ...
    const frontalArea = vehicleType === 'truck' ? 8.0 : vehicleType === 'motorcycle' ? 0.6 : 2.2;
    ...
    const dragForce = 0.5 * rho * cd * frontalArea * Math.pow(relativeWindSpeed / 3.6, 2);
...
const calculateUrbanExitTime = (urbanSegments, speedInUrban, speedInHighway) => {

...
const inviteDriver = async (req, res, next) => {
    try {
        ...
        // ✅ السطر الجديد — بيشيل أي trailing slashes من CLIENT_URL
...
const getMyVehicleStatus = async (req, res, next) => {
    try {
        ...
        if (!vehicle) {
            return res.status(200).json({ success: false, msg: 'no vehicle assigned to you yet' });
        }
...
// company_admin — assign (or replace/unassign) a driver on an existing vehicle.
// if the chosen driver is already linked to ANOTHER vehicle, he is
// automatically unassigned from it first (one vehicle per driver).
const assignDriver = async (req, res, next) => {
    try {
        ...
        if (driverId) {
            const driver = await User.findOne({ _id: driverId, role: 'company_driver', companyId });
            if (!driver) {
                return res.status(404).json({ success: false, msg: 'driver not found in your company' });
            }
            // unassign the driver from any OTHER vehicle of this company first
        ...
        res.status(200).json({
            success: true,
            msg: driverId ? 'driver assigned successfully' : 'driver unassigned',
            vehicle: await vehicle.populate('driverId', 'name email'),
        });
    } catch (err) {
        next(err);
    }
};
module.exports = { inviteDriver, addVehicle, getFleetStatus, getMyVehicleStatus, sendAlert, uploadVehiclePhoto, getDrivers, getFleetDashboard, assignDriver };

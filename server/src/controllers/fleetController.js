...
const addVehicle = async (req, res, next) => {
  try {
    ...
    if (err.code === 11000) {
      return res
        .status(409)
        .json({ success: false, msg: "plate number or driver already in use" });
    }
    next(err);
  }
};
...
const getMyVehicleStatus = async (req, res, next) => {
  try {
    ...
    if (!vehicle) {
      return res
        .status(200)
        .json({ success: false, msg: "no vehicle assigned to you yet" });
    }
...
const uploadVehiclePhoto = async (req, res, next) => {
  try {
    ...
    if (!vehicle) {
      return res
        .status(404)
        .json({ success: false, msg: "vehicle not found in your company" });
    }
  ...
  try {
    ...
    if (driverId) {
      ...
      if (!driver) {
        return res
          .status(404)
          .json({ success: false, msg: "driver not found in your company" });
      }
